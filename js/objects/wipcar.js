...
class wipaxleAndWheel extends baseObject {
	constructor(PosX, PosY, PosZ, scale) {
		super(new THREE.Vector3(PosX, PosY, PosZ));

		var geometry = new THREE.CylinderGeometry( scale*0.1, scale*0.1, scale*2.8, 10);
		if ( typeof wipaxleAndWheel.mat1 == 'undefined' || typeof wipaxleAndWheel.mat2 == 'undefined' ) { //static values
			wipaxleAndWheel.mat1 = new THREE.MeshBasicMaterial( {color: 0x808080, wireframe: false} );
			wipaxleAndWheel.mat2 = new THREE.MeshBasicMaterial( {color: 0x808080, wireframe: false} );
		}
		this.axle = new THREE.Mesh( geometry, wipaxleAndWheel.mat1 );
...
class wipcar extends physicalObject {
	constructor(PosX, PosY, PosZ, scale) {
		...
		if ( typeof wipcar.mat1 == 'undefined' || typeof wipcar.mat2 == 'undefined' ) { //static values
			wipcar.mat1 = new THREE.MeshBasicMaterial( {color: 0xaa0000, wireframe:false} );
			wipcar.mat2 = new THREE.MeshBasicMaterial( {color: 0xaa0000, wireframe:false} );
		}
		this.chassis = new THREE.Mesh( geometry, wipcar.mat1 );
		this.rearAxis = new wipaxleAndWheel(scale*1.5, -scale*0.3, 0, scale);
		this.frontAxis = new wipaxleAndWheel(-scale*1.5, -scale*0.3, 0, scale);
